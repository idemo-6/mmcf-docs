...
Как отдельный доказательный compatibility-case этот профиль рассматривается в:

- [MMCF_LINEAR_COMPATIBILITY_CASE](../../cases/MMCF_LINEAR_COMPATIBILITY_CASE.md)

Для короткого пользовательского входа в выбор правильного work object перед
чтением полного tool-profile см.:
...
| `PTSubTask` | explicit non-trivial `PhaseTransition` instance | Использовать только как дочерний объект терминального issue; никогда не трактовать как `ChangeFlow`. |
...
### 2.1 Pre-flow Delta layer

`DeltaRegistry` в этом tool-profile остается upstream-слоем относительно
terminal issue.

Нормативно:

1. `DeltaRecord` не materialize'ится в terminal issue автоматически;
2. terminal issue создается только после `PromotionDecision=promote`;
3. `DeltaRegistry` не кодируется статусом terminal issue;
4. слово `backlog` не должно использоваться как базовое имя terminal issue
   status в methodology-смысле этого профиля.
...
### 3.3 `терминальная задача -> PTSubTask`

`PTSubTask` является допустимым explicit tool object для нетривиального
`PhaseTransition` внутри существующего terminal issue.

Нормативно:

1. `PTSubTask` не является еще одним `ChangeFlow`;
2. `PTSubTask` не подменяет parent issue;
3. `PTSubTask` создается только для materially significant transition;
4. parent issue остается единственным carrier полного `CF1..CF6`.
5. selected `PT` templates привязываются к конкретному `from_phase -> to_phase`
   в parent issue, а не к issue вообще.

### 3.4 Граница контекста
...
Статусы представляют operational progression layer рабочего процесса:
...
Статусы `collect/analyze/forecast/decide/implement/evaluate` относятся только
к terminal issue.

Для явного `PTSubTask` рекомендуется отдельная малая status model, определенная
в [MMCF-Delivery-PhaseTransition-Gateway-Profile](./MMCF-Delivery-PhaseTransition-Gateway-Profile.md).

### 4.4 Pre-start semantics

- `Queued` означает, что terminal `ChangeFlow` уже materialized после
  `PromotionDecision=promote`, но еще не допущен в активное фазовое исполнение.
- `Todo` означает, что terminal `ChangeFlow` уже готов войти в `CF1`.
...
- `final` означает terminal canceled exit без ожидания дальнейшего
  возобновления линии.
- `delayed` означает terminal canceled exit с ожиданием возврата позже, когда
  изменится контекст, policy или ресурсное состояние.
...
3. `Result=-1` и `negative_result_policy` либо authority verdict на
   `PT(CF5->CF6)` выбирают немедленный corrective/compensating `CF` -> `repeat`
4. `Result=-1` и `negative_result_policy` либо authority verdict на
   `PT(CF5->CF6)` выбирают terminal stop -> `final`
5. `Result=-1` и `negative_result_policy` либо authority verdict на
   `PT(CF5->CF6)` выбирают deferred return -> `delayed`
6. `Result=0` и есть немедленно переформулированный `CF` -> `repeat`
...
1. `done` остается success-exit и используется только для `Result=+1`;
2. `final` и `delayed` являются общими canceled exits и не ограничиваются
   только `Result=0`;
3. при `Result=0` operationally используются только `repeat`, `final` и
   `delayed`.
...
8. `exit_decision`
9. `closure_reason = success | inapplicable | failed | mixed`
10. `zero_class`, когда `Result=0`
11. `next_cf_issue` для `repeat`
12. `return_condition` для `delayed`
...
3. `Work Domain`
   Допустимые значения: `artifact | meta-operational`
4. `Artifact Type`
   Допустимые значения: `software | documentation | scientific-documentation`
   Обязательно только если `Work Domain=artifact`
5. `Claim Mode`
   Допустимые значения: `claim-none | claim-optional | claim-required`

Этих пяти полей достаточно для первоначальной фильтрации и отчетности без
...
1. выставляйте `LC Phase Snapshot`, `Work Domain` и `Claim Mode` при создании
   issue;
2. выставляйте `Artifact Type` только если `Work Domain=artifact`;
3. выставляйте `Result` только в `evaluate`, когда исход потока уже известен.
...
### 6.2.1 `PTSubTask` data

Для `PTSubTask` рекомендуется отдельный, более узкий набор полей или body-block,
а не повторение full schema terminal issue.
...
5. `final` и `delayed` допустимы при `Result=0` и при `Result=-1`, если
   negative outcome closure уже разрешен через policy или authority verdict.
...
- [MMCF-Role-Composition-and-Authority-Patterns-Profile](../operational/MMCF-Role-Composition-and-Authority-Patterns-Profile.md)
