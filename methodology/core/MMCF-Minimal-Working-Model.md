...
3. `CFOwner(L, flow_id)` — owner целостного materialized `ChangeFlow` на
   уровне `L`.
4. `CFPhaseOwner(L, CFi)` — owner фаз `CF` на уровне `L`.
5. `TransitionOwner(L, PT)` — owner фазовых переходов (`LC-PT`/`CF-PT`) на уровне `L`.

Scoped precedence:

`LevelOwner/PolicyAuthority > LCPhaseOwner > CFOwner > CFPhaseOwner`

при этом precedence относится к собственному scope вопроса, а не является
абсолютной командной лестницей для любого решения.
...
4. для materialized flow фиксируется один активный `CFOwner`.
...
2. выбор исполнителя;
3. центр разрешения `authority-conflict` и конфликтов реализации;
...
6. unresolved `authority-conflict` блокирует commit, gateway closure или
   final evaluation.
