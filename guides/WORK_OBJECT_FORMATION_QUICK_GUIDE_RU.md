...
Для текущего `Linear` workspace bridge используйте такое practical mapping:

1. `Backlog` = `DeltaRegistry`, а не terminal issue;
2. materialized terminal issue кладется в `Planning` как collapsed alias
...
Практический safe rule для текущего `Linear` bridge:

- если объект лежит в `Backlog`, это еще не terminal `ChangeFlow`;
- terminal `ChangeFlow` появляется только после promotion и дальше попадает в
  `Planning`, а не в `Backlog`.
...
- [MMCF-Phase-and-Transition-State-Separation-Profile](../methodology/operational/MMCF-Phase-and-Transition-State-Separation-Profile.md)
